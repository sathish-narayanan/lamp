...
class User{
	...
	public function validate()
	{
		...
		elseif (!ctype_alpha($this->userName)) {
		...
		elseif (!ctype_digit( $this->mobileNo) OR strlen( $this->mobileNo) != 10) {
			$validate['errorMessage'] = "Enter a valid phone number";
			$validate['errorFlag'] = true;
		}
		elseif (strlen( $this->password )<self::MIN_PASSWORD_SIZE) {
		...
		elseif (strlen($this->day)>self::MAX_DAY_SIZE) {
			$validate['errorMessage'] = "enter valid day date";
			$validate['errorFlag'] = true;
		}
		elseif (strlen($this->month)>self::MAX_MONTH_SIZE) {
			$validate['errorMessage'] = "enter valid month date";
			$validate['errorFlag'] = true;
		}
		elseif (strlen($this->year)>self::MAX_YEAR_SIZE) {
			$validate['errorMessage'] = "password too weak";
			$validate['errorFlag'] = true;
		}
